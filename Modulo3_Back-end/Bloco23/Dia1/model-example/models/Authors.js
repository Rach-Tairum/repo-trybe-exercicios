...
const getAll = async () => {
	const [authors] = await connection.execute( // As [] indicam qual tabela se quer trabalhar e busca somente os dados dentro da tabela, sem informações extras
		'SELECT id, first_name, middle_name, last_name FROM model_example.authors;', // comando SQL
	);
	return authors.map((author) => serialize(author)).map(getNewAuthor);;
};
